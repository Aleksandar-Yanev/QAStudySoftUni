...
namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            Assert.Equal(townName, result.Name);
        }
        ...
        [InlineData(0)]
        [InlineData(-1)]
        public void UpdateTown_InvalidPopulation_ShoudReturn_ArgumentExeption( int invalidPopulation)
        {
            //Arrange
            string townName = "Varna";
            int population = 100000;
            ...
            // Assert
            var exeption = Assert.Throws<ArgumentException>(action);
            Assert.Equal("Population must be a positive number.", exeption.Message);
        }
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            string firstTownName = "Varna";
            int firstTownPopulation = 100000;
            _controller.AddTown(firstTownName, firstTownPopulation);

            string secondTownName = "Sofia";
            int secondTownPopulation = 2000;
            _controller.AddTown(secondTownName, secondTownPopulation);

            string thirdTownName = "Plovdiv";
            int thirdTownPopulation = 50000;
            ...
            Assert.Equal(firstTownName, listOfTowns[0].Name);
            Assert.Equal(firstTownPopulation, listOfTowns[0].Population);
            Assert.Equal(secondTownName, listOfTowns[1].Name);
            Assert.Equal(secondTownPopulation, listOfTowns[1].Population);
            Assert.Equal(thirdTownName, listOfTowns[2].Name);
            Assert.Equal(thirdTownPopulation, listOfTowns[2].Population);
        }
    }
}
